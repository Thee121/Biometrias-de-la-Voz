...
figure(5);
eje_t2 = 0:1/fs:(length(pulso_resampled)-1)/fs;
plot(eje_t2, pulso_resampled);
title('Pulso glótico');
xlabel('Tiempo (s)');
ylabel('Amplitud');

figure(6);
specgram(pulso_resampled, 512, fs);
title('Espectro del pulso glótico');

% Integramos el pulso para obtener la onda
a = [1, -0.99];
b = 0.01;
onda_glot = filter(b, a, pulso_resampled);
figure(7);
...
plot(eje, pulso_resampled, 'b', eje, onda_glot*100, 'r');
title('Pulso y onda');
xlabel('Tiempo (s)');
ylabel('Amplitud');
legend('Pulso', 'Onda');
